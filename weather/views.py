...
def index1(request):
    if request.method == 'POST':
        data1.append(json.loads(json.dumps({"level" : json.loads((request.body).decode('utf-8')).get('level'), "turb" : json.loads((request.body).decode('utf-8')).get('turb')})))
        #print(json.loads((request.body).decode('utf-8')).get('ph'))
        a = common.objects.create(level=json.loads((request.body).decode('utf-8')).get('level'),turb=json.loads((request.body).decode('utf-8')).get('turb'))
        a.save() #WE CREATE A OBJECT TO STORE SENSOR READINGS AND THEN WE STORE THOSE READING INTO THE DATABASE
        #JSON MEANS JAVA SCRIPT OBJECT NOTATION , WE CONVERTING FROM JSON TO DICTIONERY
        return JsonResponse(data1,safe=False) #SENDING DATA TO HTML PAGE
    if request.method == 'GET':
        return JsonResponse(json.loads(json.dumps(data1)), safe=False)

def plants(request): #A FUNCTION WHICH RETURNS ALL PLANTS WHEN PLANTS/ URL IS CALLED
...
def graph(request): #FUNCTION IS CALLED WHEN graph/ url is called
    ...
    p1 = common.objects.all()[::-1][0]
    # print('temp', p.moi)

    return render(request,"index.html",{'k':p, 'k1':p1}) # data is rendered to index.html in the form of dictionary
